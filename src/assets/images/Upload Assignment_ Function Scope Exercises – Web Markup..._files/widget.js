...
// The value of KLOUDLESS_APP_ID is assigned in FilePickerTag.
widget.FilePicker.KLOUDLESS_APP_ID = null;
widget.FilePicker.KLOUDLESS_BASE_URL = null;
...
widget.FilePicker.prototype =
{
    ...
    initialize : function( pickerList, baseElementName, required, overrideLocalBehavior, csPickerUrl, showAddMetadata,
                           showSpecialAction, allowMultipleFiles, onAttachFile, allowMultipleLocalFileUpload, allowCloudFileSelection, locale, showHTMLFileTypeWarning )
    {
      ...
      if ( this.allowCloudFileSelection )
    	  {
    	    // Create an instance of the Kloudless File browser JS Object.
    	    this.explorer = window.Kloudless.explorer({
    	      app_id: widget.FilePicker.KLOUDLESS_APP_ID,
    	      retrieve_token: true,
    	      multiselect: this.allowMultipleFiles,
    	      display_backdrop: true,
    	      create_folder: false,
    	      types: ['files'],
    	      base_url: widget.FilePicker.KLOUDLESS_BASE_URL
    	    });
    	    
    	    var that = this;
    	    
    	    // When user clicks the 'Browse Cloud Storage' button, launch the Kloudless file browser.
    	    $j( '#' + this.baseElementName + '_cloudBrowse' ).click(function() {
    	    	  that.explorer.update({ locale: that.locale.toLowerCase().replace(/_/g,'-') });
    	    	  that.explorer.choose();
    	    });
    	    
    	    // Tell Kloudless file browser to call our callback function, onCloudFilesPick(), when a user
    ...
    preparePickedFilesForSubmit : function( theForm )
    {
      ...
      $j("input[type=file]").each(function() {
          if($j(this).val() === "" && $j.isFunction(formData.delete) ) {
    ...
    /**
     * Callback called by Kloudless when the user picks a file from the Kloudless file browser.
     */
    ...
    {  
      ...
      this.showCloudFileDownloadIndicator( files );
      var consistsOfHtmlFile = false;
      $j(files).each(function (index, file) { 
        ...
        that.downloadCloudFileToLearn( file, accountId, id, bearerToken.key );
      });
     ...
     */
    showCloudFileDownloadIndicator: function( files )
    {
   	  ...
   	  if ( fileDownloadInd )
      {
        ...
        fileDownloadInd.show();
   		...
   		//  If not already visible, display the 'Attached files' mini-list. 
   	    //  We need this to be visible in order for the file download indicator to be visible.
     ...
     */
    hideCloudFileDownloadIndicator: function()
    { 
     ...
     */
    downloadCloudFileToLearn: function( file, accountId, id, bearerToken  )
    { 
      ...
      let endpointUrl = '/learn/api/v1/cloudstorages/Kloudless/contents/' + accountId + pipeSeparator + id + '/download?synchronousDownload=true&accessToken=' + bearerToken;
      ...
      $j.ajax(
      {
        ...
        success : function( response )
        { 
          that.hideCloudFileDownloadIndicator();
...
{
    initialize : function( baseElementName, currentAttachedFile, required, showSpecialAction, showHTMLFileTypeWarning )
    {
      this.baseElementName = baseElementName;
